...
/*   Updated: 2018/10/02 02:41:28 by lazrossi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	w_get_next(int **tmp_mat, int size, int current_index)
{
	...
	while (x < size)
	{
		if (tmp_mat[x][current_index + 1]
				&& (weight = tmp_mat[x][0]) < lighter_weight)
		{
...
void		mat_del_link(int **tmp_mat,
		int prev_node_index, int next_node_index)
{
...
void		working_mat_add_new_paths(int **working_mat, int **tmp_mat, int n)
{
	...
	{
		prev_node_index = n - 1;
		while ((next_node_index = w_get_next(tmp_mat, n + 1, prev_node_index))
				>= 0)
		{
...
void		mat_reverse_used_paths(int **tmp_mat,
